...
interface SeriesFormWizardProps {
    initialData?: SeriesFormData;
    seriesId?: string;
    isEditMode?: boolean;
}

export function SeriesFormWizard({ initialData, seriesId, isEditMode = false }: SeriesFormWizardProps) {
    const router = useRouter();
    const [currentStep, setCurrentStep] = useState(1);
    const [isSubmitting, setIsSubmitting] = useState(false);

    // Use initialData if provided, otherwise default to empty form
    ...
    const handleNextStep = async () => {
        if (currentStep < 6) {
            ...
            try {
                ...
                const endpoint = isEditMode && seriesId ? `/api/series/${seriesId}` : '/api/series';
                const method = isEditMode && seriesId ? 'PATCH' : 'POST';
                ...
                if (!response.ok) {
                    throw new Error(`Failed to ${isEditMode ? 'update' : 'schedule'} series`);
                }
    ...
    return (
        <div className="w-full max-w-5xl mx-auto pb-20">
            <div className="mb-6 flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold">{isEditMode ? "Edit Series" : "Create New Series"}</h1>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl shadow-sm p-6 md:p-10 min-h-[70vh] flex flex-col">
                ...
                <FormFooter
                    ...
                    onBack={handleBack}
                    hideBack={currentStep === 1 || isSubmitting}
                    disableNext={
                        isSubmitting ||
